...
import {
  FormsByInchargeIdDocument,
  FormsByInchargeIdQuery,
  FormsByInchargeIdQueryVariables,
  GetAllUsersDocument,
  GetAllUsersQuery,
...
interface FTDApprovalListsProps {
  inchargeID?: number;
}

export const FTDApprovalLists = (prop: FTDApprovalListsProps) => {
  //get all forms by incharge

  const [updateApprovalStatus, { loading, error }] = useMutation<
    UpdateApprovalStatusMutation,
    UpdateApprovalStatusMutationVariables
  >(UpdateApprovalStatusDocument);
  ...
  const handlechangeFormStatus = async (id: number, status: number) => {
    await updateApprovalStatus({
      variables: { formId: id, approvalStatus: status },
    });
  };

  const inchargeID = prop.inchargeID || 0;
  ...
  } = useQuery<FormsByInchargeIdQuery, FormsByInchargeIdQueryVariables>(
    FormsByInchargeIdDocument,
    { variables: { inchargeId: inchargeID } }
  );
  ...
  useEffect(() => {
    refetchForms({ inchargeId: inchargeID });
    refetchUsers();
  ...
  const unattendedForms =
    mappedForms?.filter((form) => form.approvalStatus === 1) || [];
  const pendingForms =
    mappedForms?.filter((form) => form.approvalStatus === 2) || [];
  const approvedForms =
    mappedForms?.filter((form) => form.approvalStatus === 4) || [];
  const rejectedForms =
    mappedForms?.filter((form) => form.approvalStatus === 3) || [];
  const cancelledForms =
    mappedForms?.filter((form) => form.approvalStatus === 5) || [];
  ...
  const pendingftdItems: FTDformInfo[] =
    pendingForms?.map((form, key) => ({
      key: key,
      id: Number(form.id),
      operatorName: String(form.name),
      operatorId: Number(form.operatorId),
      approvalStatus: Number(form.approvalStatus),
      shiftId: Number(form.shiftId),
    })) || [];
  const unattendedftdItems: FTDformInfo[] =
    unattendedForms?.map((form, key) => ({
      key: key,
      id: Number(form.id),
      operatorName: String(form.name),
      operatorId: Number(form.operatorId),
  ...
  const approvedftdItems: FTDformInfo[] =
    approvedForms?.map((form, key) => ({
      key: key,
      id: Number(form.id),
      operatorName: String(form.name),
      operatorId: Number(form.operatorId),
  ...
  const rejectedftdItems: FTDformInfo[] =
    rejectedForms?.map((form, key) => ({
      key: key,
      id: Number(form.id),
      operatorName: String(form.name),
      operatorId: Number(form.operatorId),
  ...
  const cancelledftdItems: FTDformInfo[] =
    cancelledForms?.map((form, key) => ({
      key: key,
      id: Number(form.id),
      operatorName: String(form.name),
      operatorId: Number(form.operatorId),
                ...
                >
                  ...
                  <List.Item title="Risky" description="Director">
                    ...
                    <Space direction="horizontal">
                      <Space>
                        {" "}
                        <Tag round>Rcn: {item.operatorId} </Tag>
                      </Space>
                      <Space>
                        {" "}
                        <Tag color="blue">
                          {" "}
                          {mapApprovalType(Number(item.approvalStatus))}{" "}
                        </Tag>
                      </Space>
                    </Space>
                  </List.Item>
